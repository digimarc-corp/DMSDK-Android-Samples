        ...
        extends Activity
{
    ...
    private static final PermissionPrompt[] mRequiredPermissions = {
    ...
    /**
     * Checks whether we have all the permissions that the app needs. To add additional permissions to
     * the checks, edit the mRequiredPermissions array.
     * @return True if we have all permissions.
     */
    static boolean haveRequiredPermissions( Context context )
    {
        boolean result = true;

        for ( PermissionPrompt prompt : mRequiredPermissions)
        {
            if (ContextCompat.checkSelfPermission(context, prompt.mPermission ) != PackageManager.PERMISSION_GRANTED)
                result = false;
        }
